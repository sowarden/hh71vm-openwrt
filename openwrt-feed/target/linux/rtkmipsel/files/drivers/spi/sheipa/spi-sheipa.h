...
struct sheipa_spi {
	...
	void *			comp_param;
	/*
	 * Sticky error for one spi_message.  The vendor call chain is
	 * do_spi_* -> select_op -> flash_* -> *_wait_busy, all of it void, so a
	 * timeout deep inside had nowhere to go and dw_spi_transfer() finished
	 * with msg->status = 0 -- a dead chip looked like a successful write.
	 * Rewriting every signature down that chain is a much bigger change than
	 * the bug is worth; the wait functions record the failure here instead
	 * and dw_spi_transfer() turns it into msg->status.
	 */
	int			io_error;
};
