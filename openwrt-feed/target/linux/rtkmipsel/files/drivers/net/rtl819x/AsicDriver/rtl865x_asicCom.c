...
/*
 * The vendor originally had this conditional include here:
 *     #if defined(CONFIG_RTL_8197F) && !defined(CONFIG_OPENWRT_SDK)
 *     #include <../bsp/bspchip.h>
 *     #endif
 * that is, under OpenWrt the register map was not connected at all. Meanwhile below (line ~1518)
 * this file calls IS_8197F_VG(), a macro from bspchip.h that reads BSP_ECO_SN. Branch
 * The vendor simply never assembled OPENWRT_SDK with this code: in the fork AC10 (snapshot
 * SDK is older) the call IS_8197F_VG is not here yet.
 *
 * The path to the header is taken the same as in rtl_nic.c: in the OpenWrt tree the register map is located
 * in arch/mips/include/asm/mach-rtl8197f/, not in a separate directory bsp/.
 */
