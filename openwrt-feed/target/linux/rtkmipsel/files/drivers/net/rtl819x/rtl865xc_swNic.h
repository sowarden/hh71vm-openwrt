...
/*
 * CONFIG_RTL_CUSTOM_PASSTHRU is disabled under OpenWrt.
 *
 * This is not a Kconfig character, but a hard #define in two vendor headers. In second place
 * (l2Driver/rtl865x_fdb.h) the vendor himself wrapped it in `#ifndef CONFIG_OPENWRT_SDK`, and here
 * forgot. The openwrt-AC10 fork left both lines as is, but its kernel tree appears to be
 * carried the vendor patch struct sk_buff.
 *
 * Why do we turn off and not drag the sk_buff patch: the IPv6/PPPoE-passthrough feature works through
 * fields srcVlanId/srcPort/srcPhyPort, which are not in the vanilla kernel - they are added by the vendor
 * patch include/linux/skbuff.h (tested against the 3.10.90 kernel from the GPL-archive DIR-842E, lines
 * 535-545), and the vendor himself announces them only with CONFIG_RTL_HARDWARE_MULTICAST /
 * QOS_VLANID / VLAN_8021Q - and we have the configuration L2-only, none of these characters
 * selected. That is, even in a vendor’s assembly with our config, these fields would not exist.
 *
 * Plus, the included passthru conflicts with swconfig: it compiles under it
 * insert_vlan_tag() from rtl_nic.c:12003, and in swconfig - part of the same file (line 27460)
 * there is a second function with the same name and a different signature.
 *
 * We don’t need the feature itself: it’s about transparent forwarding of PPPoE/IPv6 through the peth interface,
 * which is not in our configuration.
 */
