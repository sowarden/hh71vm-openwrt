...
/*
 * RTL_SKB_HAS_SRC_TAG - whether struct sk_buff has the vendor srcPort/srcVlanId fields.
 *
 * This is not our invention: the vendor declares those fields under exactly this condition in
 * its own patch to include/linux/skbuff.h (checked against kernel 3.10.90 from the GPL archive).
 * Vanilla kernel 4.14 has no such fields and we did not add them.
 * docs/rtknet-porting-map.md.
 *
 * Further down the file there are several places where the vendor writes into those fields while
 * commenting out ITS OWN guard (`//#if defined (CONFIG_RTL_HARDWARE_MULTICAST)`). The guard is
 * restored there through this macro. The writes are safe to switch off: in our configuration
 * (L2 only) nothing reads those fields - and that is not an argument but a fact proven by the
 * compiler: any live read would give the same "has no member named" error as a write.
 */
...
/*
 * get_dev_ip_mask() - our own implementation.
 *
 * The vendor declares it extern (line 741 and below) and calls it from the ACL code for
 * br-lan, while the definition lives in their kernel patch, which we do not have. The function
 * is elementary: find an interface by name and return its address and mask.
 *
 * The byte order is HOST order, as the callers require: rtl_add_Acl_for_out_of_br_subnet()
 * runs the result through htonl(), and the ARP parsing compares it with ntohl(tip). The
 * neighbouring vendor get_P2P_local_ip() behaves in exactly the same way.
 *
 * The locking is the correct one for 4.14 (RCU), unlike the neighbouring vendor function,
 * which reaches into dev->ip_ptr with no protection at all.
 *
 * Safety during bring-up: the only consumer installs an ACL only when both address and mask
 * are non-zero, so "the interface does not exist yet" or "no address is assigned" quietly and
 * correctly means "do not install an ACL".
 */
	...
	if (dev) {
		...
		if (in_dev) {
			for (ifa = in_dev->ifa_list; ifa; ifa = ifa->ifa_next) {
				/* the first address of an interface is the primary one; the label is
				   checked so that an alias such as "br-lan:1" is not picked up */
...
/*
 * MAC addresses from the hwsetting partition (H601).
 *
 * The partition sits in flash at offset 0x20000 and holds the factory addresses. We had
 * no MTD driver at the time (spi-sheipa came later), so the AC10 fork path through
 * get_mtd_device_nm("hwpart") was unavailable. Instead there is a window of DIRECT
 * READING of the SPI flash: the controller maps flash into physical memory at
 * BSP_SPIC_AUTO_BASE (0x10000000), which works with no driver, as the bootloader runs there.
 *
 * Checked on a live system BEFORE the code was written: a read at 0xB0020000 gave
 * `31303648 00019413 96814CE0 4CE000C1`, which matched a reference dump of the
 * hwsetting-H601 partition byte for byte.
 *
 * The offset of the first address inside the partition is 7 and the step is 6. That is not
 * guesswork: exactly that formula (`offset = 7 + idx * ETH_ALEN`) is used by
 * read_addr_from_flash() in the AC10 fork, giving addresses with the Realtek OUI 00:E0:4C.
 *
 * If the partition signature does not match, or an address looks unusable (zero or a group
 * address), the vlanconfig placeholder is kept: the interface still comes up.
 */
#define HH71VM_HWSETTING_OFFSET		0x20000
#define HH71VM_H601_MAC_OFFSET		7
#define HH71VM_HWSETTING_LEN		64

/*
 * UPDATED. The direct-read window stopped working once the SPI-NOR driver was ported: the
 * driver puts the controller into command mode, and 0xB0020000 no longer holds the contents
 * of flash. The symptom is "hwsetting: MAC 0 not found, keeping default" in the boot log,
 * with the interfaces coming up on the vlanconfig placeholder 00:12:34:56:78:96 instead of
 * the factory address.
 *
 * So the main path is now the hwsetting partition through MTD, and the window is kept as a
 * fallback for when MTD is not up yet. The call order allows both: the partitions are
 * registered at 5.9 s and the addresses are applied at 6.7 s.
 */
static const unsigned char *hh71vm_hwsetting(void)
{
	static unsigned char buf[HH71VM_HWSETTING_LEN];
	static int state;	/* 0 = not tried, 1 = read, -1 = failed */
	struct mtd_info *mtd;
	size_t rd = 0;
	int i;

	if (state)
		return state > 0 ? buf : NULL;
	state = -1;

	mtd = get_mtd_device_nm("hwsetting");
	if (!IS_ERR(mtd)) {
		if (mtd_read(mtd, 0, sizeof(buf), &rd, buf) == 0 && rd == sizeof(buf))
			state = 1;
		put_mtd_device(mtd);
	}

	if (state < 0) {
		const volatile unsigned char *win =
			(const volatile unsigned char *)(KSEG1ADDR(BSP_SPIC_AUTO_BASE) +
							 HH71VM_HWSETTING_OFFSET);

		for (i = 0; i < HH71VM_HWSETTING_LEN; i++)
			buf[i] = win[i];
		state = 1;
	}

	if (buf[0] != 'H' || buf[1] != '6' || buf[2] != '0' || buf[3] != '1') {
		state = -1;
		return NULL;
	}
	return buf;
...
{
	const unsigned char *p = hh71vm_hwsetting();
	int i;
	...
	if (!(mac[0] | mac[1] | mac[2] | mac[3] | mac[4] | mac[5]))
		return -1;	/* empty */
	if (mac[0] & 0x01)
		return -1;	/* a group address cannot be an interface address */
		...
		if (wan_management_vlan.vlan)
		{
			...
			if (info)
			{
				*vid = info->id;
				...
				return info->tag;     // vendor comment (originally in Chinese, lost in transcoding)
			}
		}
		#endif
	}
    ...
    if(linux_vlan_enable && linux_vlan_hw_support_enable)
    {
		...
		#endif
		{
			...
			if(ret == 1)
	        {
				...
				#endif
				/* dev->last_rx was removed from struct net_device in kernel 4.11.
				   The field was purely informational; nothing in the driver read it. */
...
Reserve_tag:
	...
	#endif
	/* dev->last_rx was removed from struct net_device in kernel 4.11 - see above. */
...
static inline int rtl_pstProcess_xmit(struct dev_priv *cp, struct net_device *dev, int len)
{
	...
	/* In kernel 4.7 dev->trans_start moved into netdev_queue; the stock replacement is
	   netif_trans_update() (include/linux/netdevice.h:3595). */
	if (dev)
...
struct proc_dir_entry *rtl_8367r_vlan;
	...
	FullAndSemiReset();
	{
		...
		*/
		...
		/* Put the real MAC addresses in place of the vlanconfig placeholders.
		   This has to happen before the net_device registration loop below: there the
		   address is copied from vlanconfig[i].mac into dev->dev_addr. */
...
//#if defined(CONFIG_RTK_VLAN_FOR_CABLE_MODEM)
	...
	/* rtl_qos_init() is defined in net/rtl/features/rtl_features.c - the directory of the
	   vendor hooks/fastpath framework, which is not in our tree. CONFIG_NET_SCHED is always
	   on in OpenWrt, so that condition alone is not enough. The AC10 fork simply comments the
	   call out; here it is disabled by an explicit condition so that the reason stays
	   visible. */
	rtl_qos_init();
#endif
#endif
                            ...
                            unsigned int flags,unsigned int status)
{
	 ...
	 if (nla_put_string(skb, IFLA_IFNAME, dev->name))
        ...
        /* Since kernel 3.19 nlmsg_end() returns void (it used to return the message length).
           The stock replacement: call it and return 0. */
        nlmsg_end(skb, nlh);
        return 0;
...
/*
 * A gap in the vendor CONFIG_SWCONFIG branch.
 *
 * rtl819x_swNic.c and rtl865xc_swNic.c call rtk_get_vlan_priority() next to
 * rtk_get_vlan_tagmask() (the latter is defined right above, in this same #ifdef
 * CONFIG_SWCONFIG block). But rtk_get_vlan_priority() itself was only put by the vendor into
 * the #ifdef CONFIG_RTL_VLAN_8021Q block (around line 25908), where it reads vlan_ctl_p - a
 * structure of the vendor 802.1Q mechanism, which does not exist in a swconfig build. The
 * result: with SWCONFIG=y and without RTL_VLAN_8021Q the build compiles but does not link.
 *
 * Return 0: in the swconfig model a VLAN has no priority field at all (see struct
 * switch_val), and a 0 in the tx_dpri field means "the default 802.1p priority" - the same
 * thing the hardware does for untagged traffic.
 */
uint8 rtk_get_vlan_priority(uint16 vid)
{
	return 0;
...
{
/* This used to be `#ifndef (CONFIG_RTL_8367R_SUPPORT || CONFIG_RTL_83XX_SUPPORT)` - which does
   not compile at all: #ifndef takes an identifier, not an expression. A vendor typo, which
   surfaced for us as "macro names must be identifiers". Written the way the bodies of the
   branches suggest it was meant. */
#if !defined(CONFIG_RTL_8367R_SUPPORT) && !defined(CONFIG_RTL_83XX_SUPPORT)
	//extern uint64 rtl865xC_returnAsicCounter64(uint32 offset);
	uint32 addrOffset_fromP0 = port * MIB_ADDROFFSETBYPORT;	
