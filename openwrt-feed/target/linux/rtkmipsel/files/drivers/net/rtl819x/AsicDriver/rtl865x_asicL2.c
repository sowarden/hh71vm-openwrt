...
/*
 * As in rtl865x_asicCom.c, the vendor guarded the register-map include with
 * !CONFIG_OPENWRT_SDK (and CONFIG_RTL819X_GPIO), but this file still calls
 * IS_8197F_VG() and IS_8197F(), which are defined in bspchip.h (lines 5709 and 8522).
 */
...
/* The external Gigabit PHY branch drives its reset line through gpio_request_one()/
   gpio_set_value() (GPIO_RESET, around line 6944) but does not include <linux/gpio.h>. The vendor
   built this branch in a tree where another include supplied the declaration. */
