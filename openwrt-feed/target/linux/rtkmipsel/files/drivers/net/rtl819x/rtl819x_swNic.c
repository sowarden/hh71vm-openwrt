...
/*
 * Both functions are defined in rtl_nic.c (lines 26158 and 27521), but are declared only there.
 * The neighboring rtl865xc_swNic.c holds exactly the same extern lines in itself (lines 137-138),
 * but here the vendor forgot them - under GCC 4.4 it was a warning, under ours GCC 7.5
 * turns that warning into an error.
 *
 * Be sure to keep them OUTSIDE of any #if: calls (lines 936, 952) are unguarded.
 */
