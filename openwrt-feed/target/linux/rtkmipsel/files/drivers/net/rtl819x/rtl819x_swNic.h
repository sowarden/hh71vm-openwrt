...
/*
 * Declarations missing from the vendor header. These four functions are defined in
 * rtl819x_swNic.c, but not declared anywhere; rtl_nic.c calls them (partly through macros
 * RTL_swNic_* from <net/rtl/rtl_types.h>). The vendor assembled GCC 4.4, where the implicit declaration is
 * warning; we have GCC 7.5 plus -Werror in EXTRA_CFLAGS, and this is a fatal error.
 * The prototypes are copied verbatim from the definitions (rtl819x_swNic.c:681, 701, 1645, 1779).
 * The danger of an implicit declaration is not only in -Werror: the compiler would consider it to be returned
 * type int, and New_swNic_freeTxRing() returns void.
 */
