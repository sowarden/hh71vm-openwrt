...
//	range->max_r_time;	/* Maximal retry lifetime */
	...
	for (i = 0; i < priv->available_chnl_num && i < IW_MAX_FREQUENCIES; i++) {
		u32 m = 0;
		/*
		 * HH71VM/OpenWrt: was `range->freq[i].i = i + 1`, that is, the number
		 * the channel was given the INDEX in the array, and not the channel itself. At 2,4 GHz it is
		 * accidentally coincided (channels 1..13 are in a row), and on 5 GHz channel 36
		 * came to userspace as “channel 1”.
		 *
		 * Because of this, two places in libiwinfo were broken:
		 * - wext_get_freqlist() puts freq[i].i directly into channel;
		 * - wext_get_hwmodelist() decides by `channel <= 14` that the radio
		 * two gigahertz, - therefore 5 GHz radio declared itself b/g.
		 * The frequency (freq[i].m) was previously counted from the correct channel.
		 */
...
/*
 * HH71VM: SIOCGIWRATE handed out an internal rate *id*, not a rate.
 *
 * find_rate() returns the driver's own rate identifier: a legacy rate is in units
 * of 500 kbit/s (54 Mbit/s == 108), an HT rate is HT_RATE_ID (0x80) + MCS, and a VHT
 * rate is VHT_RATE_ID (0xA0) + 10 * (NSS - 1) + MCS.  rtl_giwrate() multiplied that
 * number by 1000000 and called it bits per second, so libiwinfo and LuCI showed
 * "143 Mbit/s" for 2.4 GHz (0x8F, HT MCS15) and "179 Mbit/s" for 5 GHz (0xB3, VHT
 * NSS2 MCS9).  Those were index numbers wearing a unit; the radios were fine.
 *
 * The driver already knows how to turn an id into a rate -- that is how
 * current_tx_rate is printed in /proc/<ifname>/sta_info (8192cd_proc.c,
 * query_vht_rate()).  That path needs one station's stat_info; here there is no
 * station, so the radio's own bandwidth and guard interval are used, exactly as
 * 8192cd_proc.c does for cur_tx_rate.
 *
 * VHT_MCS_DATA_RATE is in units of 500 kbit/s (every reader shifts it right by one
 * to get Mbit/s), so a table entry times 500000 is bits per second.
 */
static unsigned int rtl_rate_id_to_bps(struct rtl8192cd_priv *priv, unsigned int id)
{
#ifdef RTK_AC_SUPPORT
	extern const u2Byte VHT_MCS_DATA_RATE[4][2][40];
	unsigned int bw = MIN_NUM(priv->pshare->is_40m_bw, 3);
	unsigned int sgi = (priv->pshare->ht_current_tx_info & BIT(1)) ? 1 : 0;

	if (id >= VHT_RATE_ID) {
		unsigned int idx = id - VHT_RATE_ID;

		if (idx >= 40)
			return 0;
		return VHT_MCS_DATA_RATE[bw][sgi][idx] * 500000;
	}
	if (is_HT_rate((unsigned char)id)) {
		/* HT MCS 0-7 are one stream, 8-15 are two: the same rows the VHT table
		   keeps at 0-9 and 10-19, which is why 8-15 map to 10-17. */
		unsigned int mcs = id - HT_RATE_ID;

		if (mcs >= 16)
			return 0;
		return VHT_MCS_DATA_RATE[MIN_NUM(bw, 1)][sgi][(mcs < 8) ? mcs : mcs + 2]
		       * 500000;
	}
#else
	if (id >= HT_RATE_ID)
		return 0;              /* no rate table without the VHT support built in */
#endif
	/* legacy 802.11a/b/g: the id already is the rate in units of 500 kbit/s */
	return id * 500000;
}
			       ...
			       union iwreq_data *wrqu, char *extra)
{
	...
	//printk ("txRate = %d\n", txRate);
	wrqu->bitrate.value = rtl_rate_id_to_bps(priv, txRate);
