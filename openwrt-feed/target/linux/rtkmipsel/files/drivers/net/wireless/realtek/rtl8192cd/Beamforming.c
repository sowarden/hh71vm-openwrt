	...
	)
{
	...
	if((GET_CHIP_VER(priv)== VERSION_8822B) || (GET_CHIP_VER(priv)== VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)) {
		...
		/*
		 * HH71VM: two typecasts below. This is the same case as priv->tcpack_timer:
		 * priv->tcpack_timer: in 4.14 timer_list.function is `void (*)(unsigned long)`,
		 * and both vendor functions accept `struct rtl8192cd_priv *`. It works because
		 * that the line below in .data puts the same priv and the kernel transmits it
		 * the only argument is that on MIPS o32 the calling convention matches.
		 * This block is compiled only with BEAMFORMING_SUPPORT, which is enabled
		 * turns on together with 8812FE.
		 */
