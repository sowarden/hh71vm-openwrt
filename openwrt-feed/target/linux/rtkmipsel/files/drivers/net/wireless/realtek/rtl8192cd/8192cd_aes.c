			 ...
			 const unsigned char *addr[], const int *len, unsigned char *mac)
{
	void *ctx;
	/*
	 * HH71VM: added __attribute__((aligned(4))).
	 * AES_SetKey/AES_Encrypt work with buffers as with arrays unsigned long (see their
	 * announcements below in the file), that is, they read 32-bit words. What does the vendor say about this?
	 * knows, as can be seen from the same file: before calling AES_Encrypt(_pad, _mac) he specifically
	 * checks alignment and copies to temporary buffer if necessary
	 * (“double word aligned”). For cbc/pad he did not do this and relied on the fact that
	 * the compiler will still place arrays on word boundaries. On MIPS the cost of error is
	 * unaligned access, so alignment is specified explicitly, and not “as luck would have it.”
	 */
	...
	/*
	 * HH71VM: added type cast. AES_SetKey is declared (8192cd_aes.c:328) as
	 * `unsigned long *AES_SetKey(const unsigned long *in_key, const unsigned long key_len)`,
	 * and here the key comes as `const unsigned char *`. The vendor relied on the fact that the old
	 * GCC was missing a pointer mismatch with a warning.
	 * The cast is safe: the implementation reads the key in 32-bit words, and the key buffer in
	 * in the calling code - an aligned array in the structure, not an arbitrary pointer.
	 */
	...
	while (left >= AES_BLOCK_SIZE) {
		...
		if (left > AES_BLOCK_SIZE)
			/* HH71VM: cast to the type from the declaration of AES_Encrypt (see below, ibid.
			 * reason for buffer alignment). */
	...
	/* HH71VM: cast to the type from declaration AES_Encrypt. */
...
//	unsigned char _mac[AES_BLOCK_SIZE];
	...
	/* HH71VM: cast u32* -> unsigned long*. On 32-bit MIPS it is the same width,
	 * but for the compiler these are different types. */
