...
#ifndef __ECOS
/*
 * HH71VM: Return type corrected from void to int.
 * The notifier_block.notifier_call field is declared as
 *   int (*notifier_call)(struct notifier_block *, unsigned long, void *)
 * - this is true in both 3.10 and 4.14. That is, the vendor always had a mistake here, it’s just old
 * GCC was limited to a warning, and `-w` hid it in their EXTRA_CFLAGS.
 * NOTIFY_DONE - “processed, further along the chain as usual”: the only correct answer for
 * reboot handler, which simply turns off the radio.
 */
...
// TODO: we should modify code below
	...
	if (bcnInt) {
		...
		if(priv->pshare->rf_ft_var.beacon_igi){
			/* HH71VM: type reduced to actual. ODMPTR is &priv->pshare->_dmODM,
			 * and the field is declared as `struct dm_struct _dmODM` (8192cd.h:5744).
			 * Name DM_Out_Source_Dynamic_Mechanism_Structure - from an older revision
			 * phydm; in this snapshot the structure has already been renamed, and this line
			 * didn't fix it. In addition, the variable is not used here. */
...
int_retry_process:    
    ...
    // 8.) CPWM2 //yllin
        ...
        if ( TRUE == caseCPWM2 ) {
            ...
            /*
             * HH71VM: The cast (pu4Byte) is replaced by (pu1Byte) in the four calls below.
             * Handlers are declared in WlanHAL/HalDef.h as
             *   VOID (*NicGetHwRegHandler)(HAL_PADAPTER, u1Byte RegName, pu1Byte val)
             *   VOID (*NicSetHwRegHandler)(HAL_PADAPTER, u1Byte RegName, pu1Byte val)
             * - the third argument is always pu1Byte, it is simply a “pointer to a value buffer”,
             * and how many bytes to read/write is decided by the RegName handler itself. Neighboring
             * The calls two lines above (HW_VAR_REG_CCK_CHECK) are written like this by the vendor.
             * Only the pointer type changes, the address and behavior are the same.
             */
...
//			priv->ps_ctrl = 0x11;
		...
		if (priv->pmib->miscEntry.tcpack_acc)
		{
			...
			/*
			 * HH71VM: added type cast. timer_list.function to 4.14 —
			 * `void (*)(unsigned long)`, and check_tcp_ack_timeout is declared as
			 * `void check_tcp_ack_timeout(struct rtl8192cd_priv *priv)`
			 * (8192cd_headers.h:893). This works because the line above in .data
			 * the same priv is put in, and the kernel passes it as a single argument -
			 * that is, on MIPS the calling convention matches. All other timers
			 * drivers are designed the same way. The cast makes the intent explicit to the compiler;
			 * the signature of the function itself would have to be changed in all its calls.
			 */
...
#endif
/*
 * HH71VM: iterate over ALL radios, not only radio zero.
 *
 * SYMPTOM. With 8812FE (`CONFIG_USE_PCIE_SLOT_0`) enabled, the chip was correctly listed on the bus
 * (`Find PCIE Port, Device:Vender ID=f81210ec`, BAR's distributed), driver loading, HALMAC
 * started (`MACHAL_version_init` in the log) - but the `wlan1` interface did not appear, and in everything
 * there was not a single line in the log about 8812F. Only SoC-radio 8197F was up.
 *
 * CAUSE. The branch is selected by `CONFIG_WIRELESS_LAN_MODULE`, and we have the driver assembled
 * as a MODULE so development iterations can use scp+insmod without a power cycle while
 * holding WPS). That's why:
 * - first branch (`for` for all devices) - only for the compiled driver;
 * - the second (`for`) requires `RTK_NL80211`, and the entire path cfg80211 is turned off: it requires
 * `CPTCFG_CFG80211_MODULE`, that is, the assembly inside backports/mac80211, which we do not have;
 * - the third remains - `if (wlan_index < ARRAY_SIZE(...))` with `wlan_index == 0`, that is
 * EXACTLY ONE device is initialized, `wlan_device[0]`. And we have zero - SoC-radio
 * (`TYPE_EMBEDDED`, branch `CONFIG_WLAN_HAL_8197F && CONFIG_BAND_2G_ON_WLAN0`), the same map
 * on PCIe lies in `wlan_device[1]`.
 * `wlan_index` is not a modular parameter (for the vendor it’s just `static int`), so
 * There is nothing externally that can influence the choice of device.
 *
 * WHY DO WE DO THIS EXACTLY? An alternative is to build the driver compiled, then branch
 * will choose herself. But `CONFIG_WIRELESS_LAN_MODULE` controls a dozen more places (see.
 * 8192cd_cfg.h: memory pool sizes, delayed initialization, etc.), that is, this is a change
 * an entire operating mode for the sake of one line, plus the loss of a fast debugging cycle through insmod.
 * Here we do exactly what the vendor does in the compiled version.
 *
 * The unloading function (`rtl8192cd_exit`) traverses the ENTIRE array `wlan_device[]` without editing
 * with its own counter, so that the removal of all radios is already coordinated with this cycle.
 */
