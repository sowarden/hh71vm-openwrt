...
//    pHalFunc->ReleaseShortCutTxDescHandler  = ReleaseShortCutTxDesc88XX;
    ...
    /*
     * HH71VM: added cast to field type.
     * Field type (HalDef.h:673): ... u4Byte direction, u4Byte index
     * Implementation (Hal88XXTxDesc.h:719): ... u4Byte direction, BOOLEAN useHW
     * — the last parameter from the vendor has moved between the pointer ad and the
     * function. On MIPS o32 both arguments are passed by a single 32-bit register, so
     * the actual behavior of the cast does not change: it is exactly the same as in the stock
     * firmware, where the same code is compiled by the old GCC without diagnostics.
     * They did not tidy up the signatures themselves on bring-up - this is a general edit for everyone
     * chips in the headline, its price is higher than its benefits. The same line is in Hal8812FEGen.c
     * because both chips require it.
     */
    ...
    /*
     * HH71VM: added cast to field type.
     * HalGeneralDummy is a stub with one argument (HalCommon.h:54), and the field is declared as
     * three-argument handler (HalDef.h:1136). The vendor inserts this stub wherever
     * the chip does not have a corresponding operation; the function simply doesn’t read extra arguments, what’s on
     * MIPS o32 safe. The behavior does not change - it is only explicitly told to the compiler.
     */
