...
int DOT11_Process_Disconnect_Req(struct net_device *dev, struct iw_point *data)
{
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		/*
		 * HH71VM: was `__DOT11_Process_Disconnect_Req(dev, data)`.
		 * The function takes DOT11_DISCONNECT_REQ*, and `data` is struct iw_point*
		 * (wrapper wext), payload lies in data->pointer. Vendor in two
		 * in other places of the SAME file it writes correctly:
		 *   8192cd_security.c:2505  __DOT11_Process_Disconnect_Req(dev, (DOT11_DISCONNECT_REQ *)(data->pointer))
		 * I just forgot here. We bring them to their same form.
		 * Without the edit, the function would read MACAddr at the offset inside iw_point.
		 */
