...
/*
 * HH71VM: pre-declarations of structure tags.
 *
 * Below, in struct hci_ops_s, the tags rtl8192cd_priv / tx_insn / stat_info appear for the first time
 * INSIDE the parameter list of pointer functions. According to C rules, the area receives such a tag
 * visibility of the prototype: it does NOT coincide with the structure of the same name declared later at the level
 * file. This results in two different types with the same name, and any call through hci_ops gives
 *   error: passing argument 1 ... from incompatible pointer type
 *   note: expected 'struct rtl8192cd_priv *' but argument is of type 'struct rtl8192cd_priv *'
 * - a message that looks meaningless until you know about the scope of the prototype.
 *
 * The vendor didn’t come up with this for two reasons at once: EXTRA_CFLAGS contained `-w` (we
 * removed, see Makefile), and in the 3.x/4.4 kernels there was only a mismatch of pointer types
 * warning. In 4.14 the main Makefile kernel adds -Werror=incompatible-pointer-types,
 * and this is already a mistake.
 *
 * Minimal editing and no consequences for the code: three lines of tag declaration at the file level
 * before the first mention. It doesn't change any logic, it just makes the type the same.
 */
