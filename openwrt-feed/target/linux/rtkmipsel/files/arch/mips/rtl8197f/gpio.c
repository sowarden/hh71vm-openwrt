...
/*
 * Here AC10 under #ifdef CONFIG_OPENWRT_SDK had global gpio_to_irq() and
 * irq_to_gpio(), both with body `/ * FIXME * / return -EINVAL;`. They were removed when
 * CONFIG_OPENWRT_SDK was needed by the vendor rtknet:
 *
 * - in kernel 4.14 gpio_to_irq() is static inline in <linux/gpio.h>, so global
 * a definition with the same name breaks the assembly;
 * - irq_to_gpio() from the 4.14 kernel has been completely removed, there is no one to call it;
 * - there was no use for stubs returning -EINVAL on any board: working
 * the conversion gives the chip method rtl819x_gpio_to_irq() above.
 */
