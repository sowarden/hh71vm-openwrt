 ...
 * vendor (SDK v3.6.0, core 4.4) - base. Only he has the entry MDIO
 * 0x08 = 0x3101 under IS_8197F_VG() at reference 40 MHz: for the chip version VG
 * PHY PCIe has its own setting, and without it the link on the VG crystal may not rise;
 * AC10 - exported PCIE_link_ok / PCIE_dev_id, which are read by the WiFi driver.
 *
 * EXPORT_SYMBOL(PCIE_reset_procedure_97F) is left unconditional, like the vendor:
 * AC10 hid it behind CONFIG_OPENWRT_SDK, but we do not select this symbol
 * (why - see Kconfig). The PCIe card driver always needs export.
 */
...
/*
 * Taken from version AC10 (the vendor does not have these two variables).
 *
 * They are read by the WiFi driver rtl8192cd when the device is connected to PCIe:
 * 8192cd_osdep.c declares both symbols extern and uses them to decide whether
 * reset procedure PCIE_reset_procedure_97F(). We have RTL8812FE hanging on PCIe
 * on this board, so the symbols must exist or the driver will not link.
 *
 * PCIE_link_ok — the link went up on its own at the time of initialization;
 * PCIE_dev_id is the raw word Device:Vendor ID from the configuration space.
 */
int PCIE_link_ok = 0;
