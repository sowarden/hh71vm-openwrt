...
/*
 * .name specifies the name of the mtd device and must match the prefix in mtdparts
 * from the kernel command line ("mtdparts=m25p80:...").
 *
 * .type is intentionally not specified here. In the kernel 4.14 m25p80 passes it to
 * spi_nor_scan() as the name of the chip, and in the spi_nor_ids table the entry "m25p80" is
 * real ST M25P80 on 1 MiB. Name matching disables auto-detection
 * by JEDEC ID, and a flash on 16 MiB would be declared of size 1 MiB: no marking
 * fits and root=/dev/mtdblock4 has nothing to mount.
 */
...
int __init plat_spi_init(void)
{
	...
	/*
	 * rtk_init_flash_proc() is not called intentionally.
	 *
	 * She sets up vendor /proc/flash/hwpart and /proc/flash/bootoffset and
	 * reads bootinfo double image. We do not use a double image, but
	 * both offsets are known from the kernel command line markup, so
	 * there is no functional loss.
	 *
	 * Practical reason: with this call, the kernel loaded from flash
	 * reproducibly crashes shortly after start in an extraneous thread
	 * (Oops in kswapd0, finish_wait+0x8, current == NULL). The same core
	 * submitted by TFTP works, but without CONFIG_SPI_SHEIPA there is no crash at all.
	 * Disabling just reading bootinfo does not help - it means direct
	 * access to the mmap flash window is not to blame. Mechanism [UNVERIFIED]:
	 * details and verification method - docs/spi-nor-driver-porting.md.
	 */
