...
/*
 * Margin for work area LZMA (array of probabilities). It depends on lc/lp
 * from the stream header and for our parameters takes about 16 KB; we take from
 * large reserve, an extra cache reset costs nothing.
 */
...

{
	...
	/*
	 * The cache needs to be reset in the area of ​​loader itself. It runs in
	 * KSEG0, that is, through the cache, and leaves dirty lines in its code,
	 * stack and workspace LZMA. For the kernel, this is normal free memory:
	 * it puts its structures there, and when dirty lines later
	 * are forced out, they overwrite someone else's data.
	 *
	 * When LZMA_TEXT_START = 0x84000000 a miss is unlikely and the bug remains
	 * invisible, but the flash image is linked via 0x80A00000: the kernel reaches
	 * to this memory almost immediately after the start, and the damage is reproducible
	 * breaks waitlists - drop comes from kswapd.
	 */
