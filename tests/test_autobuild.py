...
class PublicationTests(unittest.TestCase):
    ...
    def test_interrupted_upload_resumes_exact_draft(self):
        ...
        self.assertEqual(sum(endpoint == "releases" and method == "POST"
                             for endpoint, method in self.github.mutations), 1)
        self.assertLess(self.github.mutations.index(("git/refs", "POST")),
                        self.github.mutations.index(("releases", "POST")))
    ...
    def test_matching_or_new_tag_can_create_release(self):
        self.github.ref = {"object": {"type": "commit", "sha": COMMIT}}
        self.publish()
        self.assertIsNotNone(self.github.release)
        self.assertEqual(sum(endpoint == "git/refs" for endpoint, _ in self.github.mutations), 0)
